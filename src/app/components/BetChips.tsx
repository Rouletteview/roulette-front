...
interface Props {
  setOpenBetModal: React.Dispatch<React.SetStateAction<boolean>>;
...
const BetChips = ({ setOpenBetModal, selectedChip, onChipSelect }: Props) => {
    ...
    <div className="flex flex-col gap-y-3.5">
      {chips.map((chip) => (
        <button
          ...
          className={`relative w-10 lg:w-16 hover:scale-125 h-auto transition-all cursor-pointer rounded-full ${selectedChip === chip.value ? ' scale-125' : ''
            }`}
        >
